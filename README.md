# The Divan and the Sword (TDS)

![License](https://img.shields.io/badge/License-CC%20BY--SA%204.0-blue.svg)
![Type](https://img.shields.io/badge/Type-Scholarly%20Research-darkgreen.svg)
![Approach](https://img.shields.io/badge/Approach-Evidence--Based-orange.svg)

**An independent scholarly research project on Iranian political tradition, bureaucracy, governance, and the Persianate world.**

---

## 📖 About the Project

**The Divan and the Sword (TDS)** is an independent scholarly research project dedicated to the study of political authority, administrative institutions, governance, and the intellectual heritage of the Iranian and Persianate world.

The project adopts an **evidence-based approach** grounded in transparency, traceability, and scholarly rigor. It integrates historical research, critical source analysis, and digital humanities methods to explore institutional continuity, political thought, and state formation across Iranian history.

---

## 🎯 Research Areas

- Iranian political tradition
- Bureaucracy and administrative history
- Governance and institutional continuity
- Persianate intellectual and cultural history
- Historical source criticism
- Digital humanities

---

## 🌐 Official Website

Visit the official project website:

**https://thedivanandthesword.github.io/tds-project/**

---

## 📁 Repository Structure

```text
tds-project/
├── _config.yml
├── index.md
├── about.md
├── research.md
├── book.md
├── documents.md
├── roadmap.md
├── CONTACT.md
├── CITATION.cff
├── LICENSE
├── assets/
├── _layouts/
└── docs/
```

---

## 📌 Project Status

- **Development Status:** Active
- **Documentation:** Complete
- **Governance:** Stable
- **Publication:** In preparation

---

## 🔍 Governance Principles

The project follows several core principles:

1. Evidence-based scholarship.
2. Transparency and traceability.
3. No status change without objective evidence.
4. Long-term sustainability.
5. Open scholarly documentation.

---

## 📬 Contact

Questions, comments, and scholarly feedback are welcome.

For complete contact information, please visit the **Contact** page.

### Project

- **Website:** [Official Website](https://thedivanandthesword.github.io/tds-project/)
- **Email:** [divan.and.sword@gmail.com](mailto:divan.and.sword@gmail.com)
- **GitHub:** [thedivanandthesword/tds-project](https://github.com/thedivanandthesword/tds-project)

### Project Lead

- **Hamid Reza Asadian Dehaghani**
- **ORCID iD:** [0009-0001-5921-5698](https://orcid.org/0009-0001-5921-5698)

For Facebook, Instagram, and additional contact details, see the **Contact** page on the official website.

---

## 📖 Citation

Citation metadata is available in the **CITATION.cff** file.

---

## 📄 License

Unless otherwise noted, the scholarly content, documentation, and website materials in this repository are licensed under the **Creative Commons Attribution–ShareAlike 4.0 International (CC BY-SA 4.0)** License.

See the **LICENSE** file for details.
